...
std::string get_cxx_includes() {
    ...
    if (sys_includes.length() > 0) {
        ...
        // based on https://stackoverflow.com/questions/2896600/how-to-replace-all-occurrences-of-a-character-in-string
        size_t start_pos = 0;
        while((start_pos = sys_includes.find(from, start_pos)) != std::string::npos) {
...
void verify(const Tin a[COMPLEX][M][K], const Tin b[COMPLEX][N][K], const Tout c[COMPLEX][M][N]) {
    ...
    for (unsigned m = 0; m < M; m++) {
        for (unsigned n = 0; n < N; n++) {
            if (errs >= max_errs) continue; // break is not allowed in omp loop, this makes the loop exit quickly instead
            std::complex<Tout> sum = 0;
            for (unsigned k = 0; k < K; k++) {
                // assume A row major and B col major
                // NOTE: a & b are converted to output data type (e.g. float for input type half)
...
int main() {
    ...
    using A_trans_t = Tin[beams / beams_per_block][samples / samples_per_wmma][COMPLEX][beams_per_block][samples_per_wmma];
    using B_trans_t = Tin[frames / frames_per_block][samples / samples_per_wmma][COMPLEX][frames_per_block][samples_per_wmma];
    ...
    for (int idx = 0; idx < complex * beams * samples; idx++) {
        static_cast<Tin *>(h_a)[idx] = __float2half(16 * ((float)rand() / RAND_MAX) - 8);
    }
    for (int idx = 0; idx < complex * frames * samples; idx++) {
        static_cast<Tin *>(h_b)[idx] = __float2half(16 * ((float)rand() / RAND_MAX) - 8);
    }

    // We start with a transpose kernel to get the data in the right shape for the GEMM.
    // The original data is allocated on the GPU in a subscope because we do
    // not need it anymore  after the transpose.
    // When DeviceMemory goes out of scope, the destructor will free the GPU memory
    ...
    // Transpose A
    {
        ...
        A_trans_t *d_a_trans_arg = reinterpret_cast<A_trans_t *>(static_cast<CUdeviceptr>(d_a_trans));

        transpose<<<grid, threads, 0, stream>>>(*d_a_trans_arg, *d_a_arg);
        stream.synchronize(); // to ensure d_a is not freed before transpose has finished
    }
    ...
    // Transpose B
    {
        ...
        B_trans_t *d_b_trans_arg = reinterpret_cast<B_trans_t *>(static_cast<CUdeviceptr>(d_b_trans));

        transpose<<<grid, threads, 0, stream>>>(*d_b_trans_arg, *d_b_arg);
        stream.synchronize(); // to ensure d_b is not freed before transpose has finished
    }
    ...
    std::cout << "Problem size (M, N, K): (" << beams << ", " << frames << ", " << samples << ")" << std::endl;
    std::cout << "Thread block size: (" << threads.x << ", " << threads.y << ", " << threads.z << ")" << std::endl;
    std::cout << "Threads per block: " << threads.x * threads.y * threads.z << std::endl;
    ...
    const std::string kernel(&_binary_gemm_kernel_cu_start, &_binary_gemm_kernel_cu_end);
    ...
    std::vector<const void *> parameters = {d_c.parameter(), d_a_trans.parameter(), d_b_trans.parameter()};
